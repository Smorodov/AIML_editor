...
AIMLParser::AIMLParser(bool displayTree) : displayTree(displayTree)
{
    ...
    logStream = new QTextStream(&logString);
...
bool AIMLParser::loadAimlFromString(const QString& data)
{
    ...
    {
        (*logStream) << QString("Error while parsing : %1 (line %2 - col %3)\n").arg(msg).arg(line).arg(col);
        return false;
    }
...
QString AIMLParser::executeCommand(const QString& commandStr)
{
    ...
    si.wShowWindow = 0;
    QString qstr = QString("cmd.exe /c \"%1\"").arg(commandStr);
    const char* str = qstr.toUtf8();
    create = CreateProcessA(NULL,                                   // The full path of app to launch
        LPSTR(qstr.toStdString().c_str()),  // Command line parameters
    ...
    if (!create)
    {
        (*logStream) << spaceIndent + "Cam't create prpcess!\n";
        return "";
    }
    ...
    if (!fd)
    {
        (*logStream) << spaceIndent + "Can't open pipe for write !\n";
        return "";
    }
    ...
    if (!file)
    {
        (*logStream) << spaceIndent + "Can't open file stream !\n";
        return "";
    }
#else
